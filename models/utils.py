...
def conv2d(input, weight, padding=0, sample_wise=False):
    ...
    if sample_wise:
        ...
        #print('input', input.size()) #input torch.Size([16, 64, 132, 132]) x
        #print('weight', weight.size()) #weight torch.Size([16, 3, 64, 5, 5]) d
        #print('input_sw', input_sw.size()) #input_sw torch.Size([1, 1024, 132, 132]) x
        #print('weight_sw', weight_sw.size()) #weight_sw torch.Size([48, 64, 5, 5]) d
        #print("pad(input_sw, padding, mode=circular", pad(input_sw, padding, mode='circular').size())
        #pad(input_sw, padding, mode=circular torch.Size([1, 1024, 132, 132])
        #print('out', out.size()) #out torch.Size([16, 3, 128, 128])
    else:

...
class Model:
    ...
    def load_network(self, load_path: str, network: Union[nn.DataParallel,
        ...
        state_dict_xnlb = torch.load(load_path + 'xnlb.pth')
        network.xnlb.load_state_dict(state_dict_xnlb, strict=True)

        state_dict_soft_thrink = torch.load(load_path + 'soft_thrink.pth')
        network.body_m.soft_thrink.load_state_dict(state_dict_soft_thrink, strict=True)
    ...
    def save(self, logger: Logger, current_step):
        logger.info('Saving the model.')
        net = self.net
        if isinstance(net, nn.DataParallel):
            net = net.module
        self.save_network(net.body.net_x, 'x_{}'.format(current_step))
        self.save_network(net.hypa_list, 'hypa_{}'.format(current_step))
        self.save_network(net.head, 'head_{}'.format(current_step))
        self.save_network(net.body.net_d, 'd_{}'.format(current_step))

        self.save_network(net.body_m.soft_thrink, 'soft_thrink_{}'.format(current_step))
        self.save_network(net.xnlb, 'xnlb{}'.format(current_step))
    ...
    def feed_data(self, data: Dict[str, Any]): #data 里面有y，y_gt,sigma,path
        ...
        self.sigma_y = data['sigma_y'].to(self.device)
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            h_y, w_y = y.size()[-2:]
            top = slice(0, h_y // 8 * 8)
            left = slice(0, (w_y // 8 * 8))
            y = y[..., top, left]

            m = self.m
            h_m, w_m = m.size()[-2:]
            top = slice(0, h_m // 8 * 8)
            left = slice(0, (w_m // 8 * 8))
            m = m[..., top, left]

            self.dx1, self.d = self.net(y, self.sigma_y, m)
    ...
    #保存可视化
    def save_visuals(self, tag_x: str):
        ...
        old_img_path = os.path.join(img_dir, f"{img_name:s}_{tag_x}_*_*.png")
        ...
        img_path = os.path.join(
            img_dir,
            f"{img_name}_{tag_x}_{self.metrics['psnr']}_{self.metrics['ssim']}.png"
        )
        ...
        if self.opt['test']['visualize']:
            ...
            util.imsave(y_img, img_path.replace('.png', '_input_y.png'))
            util.imsave(dx_img,  img_path.replace('.png', '_output_y.png'))


    def train(self):

        for params in [self.net.module.head.parameters(), self.net.module.body.parameters(),self.net.module.hypa_list.parameters()]:
            for param in params:
                param.requires_grad = False
        self.optimizer.zero_grad()

        dxs, self.d = self.net(self.y, self.sigma_y, self.m)

        # loss

        # loss = self.cal_multi_loss(dxs, self.y_gt)
        loss = self.cal_multi_loss(dxs, self.y_gt) # + 0.01 * self.cal_multi_loss(mx, self.m)
